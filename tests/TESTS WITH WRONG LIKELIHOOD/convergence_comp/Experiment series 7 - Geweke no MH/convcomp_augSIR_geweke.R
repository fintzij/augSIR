...
gillespie_trajecs[[1]] <- build_countmat(SIRres$trajectory, popsize)
...
results <- list(trajectories, gillespie_trajecs)
...
pdf(file = paste(paste("convcomp_plots_augSIR",popsize,censusInterval,samp_prob,R0,initialization_num,sep="_"),".pdf", sep=""))
    ...
    augSIR_trajecs <- matrix(0, nrow = length(times), ncol = niter)
    gillespie_trajecs <- matrix(0, nrow = length(times), ncol = niter)
    ...
    for(s in 600:niter){
        ...
        for(t in 1:length(times)){
            augSIR_trajecs[t,s] <- augSIR_traj[sum(augSIR_traj[,"time"] <= times[t]), "numsick"]
            gillespie_trajecs[t,s] <- gillespie_traj[sum(gillespie_traj[,"time"] <= times[t]), "numsick"]
        }
    }
    
    results_comp <- data.frame(method = rep(c("augSIR", "Gillespie"), each = length(times)), time = rep(times, 2), count = 0)
    results_comp[1:length(times),"count"] <- rowMeans(augSIR_trajecs[,600:1000])
    results_comp[(1+length(times)):nrow(results_comp),"count"] <- rowMeans(gillespie_trajecs[,600:1000])
