...
augSIR[,1] <- W.cur[,3]
gillespie[,1] <- SIRsim(popsize = popsize, initdist = initdist, b = b, mu = m, a=0, tmax = tmax, censusInterval = censusInterval, sampprob = samp_prob, returnX = TRUE, trim = FALSE)$results$Truth
extended[,1] <- SIRsim_extended(popsize = popsize, initdist = initdist, b = b, m = m, obstimes = obstimes, samp_prob = samp_prob, trim = FALSE)$results$Truth
...
for(k in 2:niter){
    ...
    augSIR[,k] <- W.cur[,3]
    gillespie[,k] <- SIRsim(popsize = popsize, initdist = initdist, b = b, mu = m, a=0, tmax = tmax, censusInterval = censusInterval, sampprob = samp_prob, returnX = TRUE, trim = FALSE)$results$Truth
    extended[,k] <- SIRsim_extended(popsize = popsize, initdist = initdist, b = b, m = m, obstimes = obstimes, samp_prob = samp_prob, trim = FALSE)$results$Truth
...
ggplot(subset(results_comp, method != "Gillespie Extended"), aes(x = time, y = count, colour = method)) + geom_ribbon(aes(ymin = count - 1.96 * mcse, ymax = count + 1.96 * mcse, fill = method, alpha = 0.1)) + geom_line() + theme_bw() + labs(title = "augSIR vs. Gillespie")
