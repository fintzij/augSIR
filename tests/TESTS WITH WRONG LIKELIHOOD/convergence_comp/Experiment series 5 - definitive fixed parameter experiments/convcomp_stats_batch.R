...
for (popsize in c(50, 150, 300)) {
    for (censusInterval in c(0.05, 0.2)) {
        for (samp_prob in c(0.25)) {
            for (resample_prop in c(1)) {
                for (initialization_num in 1:9) {
                    for(R0 in c(4)){
                        k = k+1; j = j+1
                        rbatch("./convcomp_augSIR_fixedpar.R", seed = k, popsize = popsize, censusInterval = censusInterval, samp_prob = samp_prob, resample_prop = resample_prop, initialization_num = initialization_num, R0 = R0)
                    }
                }
            }
        }
