...
SIRres <- SIRsim_extended(popsize = popsize, initdist = initdist, b = b, m = m, obstimes = obstimes, samp_prob = samp_prob, returnX = TRUE)
if(sum(SIRres$results$Observed) < 10){
    while(sum(SIRres$results$Observed) < 10){
        SIRsim_extended(popsize = popsize, initdist = initdist, b = b, m = m, obstimes = obstimes, samp_prob = samp_prob, returnX = TRUE)
    }
}
...
par(mfrow = c(1,1))
pairs(results[[2]][seq(1,niter,by=5),c(1:5, ncol(results[[2]]))])
