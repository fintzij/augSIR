...
setwd("C:/Users/Jonathan/Google Drive/UW/Year 3 +/Dissertation/Code/augSIR/tests/convergence_comp/Experiment series 5 - definitive fixed parameter experiments/Results data - corrected likelihood")

# Collect data for each R0 ------------------------------------------------

files <- list.files()

simsettings <- expand.grid(simnum = 1:3, popsize = c(50, 150, 300), censusInterval = c(0.05, 0.2), samp_prob = c(0.25), R0 = c(4))

results <- cbind(simsettings,data.frame(loglik = NA, accepts = NA, time = NA))
...
for(k in 1:nrow(results)) {
    ...
    set.seed(results[,"simnum"][k] + 9235)
...
plotcombns <- expand.grid(popsize = c(50, 150, 300), censusInterval = c(0.05, 0.2), samp_prob = c(0.25), R0 = c(4))
