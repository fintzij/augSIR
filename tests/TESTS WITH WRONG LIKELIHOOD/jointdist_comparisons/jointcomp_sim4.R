...
for(k in 1:samp_size){
    ...
    # simlate dataset
    SIRres<-SIRsim(popsize = popsize, initdist = initdist, b = b, mu=m, a=0, tmax = tmax, censusInterval=0.05, sampprob = 0.5, returnX = TRUE, trim = FALSE)
    ...
    # build irm matrices
    pathirm.cur <- build_irm(Xcount = Xcount.cur, b = b, m = m, a = 0, pop = FALSE)
...
print(ggplot(path_comp, aes(x=time, y = infected, colour = method)) + geom_line() + labs(title = "Average number of infecteds."))
dev.off()

png("infecprob_demeaned_sim4.png", width = 750, height = 550)
print(ggplot(path_comp2, aes(x=time, y = infected, colour = method)) + geom_line() + labs(title = "Average number of infecteds. Subtracting Gillespie means."))
dev.off()
