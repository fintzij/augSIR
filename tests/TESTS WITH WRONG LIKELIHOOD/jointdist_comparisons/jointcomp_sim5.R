...
for(k in 1:samp_size){
    ...
    for(j in 1:length(subjects)){
        ...
        if(j %% popsize == 1) W.cur[,2] <- rbinom(nrow(W.cur), W.cur[,3], samp_prob)
...
png("infecprob_sim5.png", width = 750, height = 550)
...
png("infecprob_demeaned_sim5.png", width = 750, height = 550)
