...
niter <- 3000000
trajecs_every <- 1
times <- seq(0, tmax, by = censusInterval)
...
augSIR[,1] <- W.cur[,3]
gillespie[,1] <- SIRsim(popsize = popsize, initdist = initdist, b = b, mu = m, a=0, tmax = tmax, censusInterval = censusInterval, sampprob = samp_prob, returnX = TRUE, trim = FALSE)$results[,3]
extended[,1] <- SIRsim_extended(popsize = popsize, initdist = initdist, b = b, m = m, obstimes = obstimes, samp_prob = samp_prob, trim = FALSE)$results[,3]
...
for(k in 2:niter){
    ...
    augSIR[,k] <- W.cur[,3]
    gillespie[,k] <- SIRsim(popsize = popsize, initdist = initdist, b = b, mu = m, a=0, tmax = tmax, censusInterval = censusInterval, sampprob = samp_prob, returnX = TRUE, trim = FALSE)$results[,3]
    extended[,k] <- SIRsim_extended(popsize = popsize, initdist = initdist, b = b, m = m, obstimes = obstimes, samp_prob = samp_prob, returnX = TRUE, trim = FALSE)$results[,3]
}
end.time <- Sys.time()

total.time <- difftime(end.time, start.time, units = "mins")

results <- list(augSIR, gillespie, extended)
...
results_comp <- data.frame(method = rep(c("augSIR", "Gillespie Counts", "Gillespie Extended"), each = length(times)), time = rep(times, 3), count = 0, mcse = 0)
...
augSIR_vars <- sapply(apply(augSIR, 1, initseq), "[[", "var.pos")
results_comp[results_comp$method == "augSIR","mcse"] <- sqrt(augSIR_vars)/sqrt(niter)
results_comp[results_comp$method == "Gillespie Counts","mcse"] <- apply(gillespie, 1, sd)/sqrt(niter)
results_comp[results_comp$method == "Gillespie Extended","mcse"] <- apply(extended, 1, sd)/sqrt(niter)

ggplot(results_comp, aes(x = time, y = count, colour = method)) + geom_ribbon(aes(ymin = count - 1.96 * mcse, ymax = count + 1.96 * mcse, fill = method, alpha = 0.1)) + geom_line() + theme_bw() + labs(title = "augSIR vs. Gillespie")
