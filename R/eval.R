...
SIRres<-SIRsim(N = 200, S0 = 199, I0 = 1, b = 0.01, mu=.5, a=0, maxtime = 300,censusInterval=1);
...
X.cur <- initializeX(W, Mu[1], probs[1], 2, tmax=20)
...
for(k in 1:(niter-1)){
  ...
  trajectories <- sample(unique(X.cur[,2]),length(unique(X.cur[,2])),replace=TRUE)
  irm.cur <- buildirm(X.cur, b = Beta[k], m = Mu[k], a = Alpha[k]) 
  
  for(j in 1:length(trajectories)){
    Xother <- X[X[,2]!=trajectories[j],]; 
    if(checkpossible(Xother) == TRUE){
        
        path.cur <- getpath(X.cur, j)
        
        W.new <-updateW(W.cur, Xother)
        irm.other <- buildirm(Xother, b=Beta[k], m = Mu[k], a=Alpha[k])
        Xt <- drawXt(Xother = Xother, irm = irm.other, W=W, p=probs[k], b=Beta[k], m=Mu[k], a=Alpha[k], initdist = initdist)
        
        path.new<- drawpath(Xt, Xother, irm, tmax)
        
        X.new <- updateX(X,path.new,j)
        irm.new <- buildirm(X.new, b = Beta[k], m = Mu[k], a = Alpha[k])
        
        a.prob <- pop_prob(X.new, irm = irm.new) - pop_prob(X.cur, irm = irm.cur) + traj_prob(path.cur, irm.other) - traj_prob(path.new, irm.other)
        if(min(a.prob, 0) > runif(1,log=TRUE)) {
            X.cur <- X.new
            W.cur <- W.new
            irm.cur <- irm.new
        }
