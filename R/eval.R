...
SIRres<-SIRsim(N = 200, S0 = 199, I0 = 1, b = 0.01, mu=.5, a=0.0005, maxtime = 300,censusInterval=1);
...
dat <- data.frame(SIRres); dat$infected<-rbinom(n=dim(dat)[1], size=dat$infected, prob=.2)
...
initdist <- c(0.999,0.001,0) # initial distribution for individual infection status
...
Alpha <- vector(length = niter); Alpha[1] <- 0.0005 + runif(1, -0.00005, 0.00005)
...
# observation matrix
W <- as.matrix(data.frame(time = dat$time, sampled = dat$infected, augmented = 0)); 
if(W[1,2]==0) W[1,2]<-1
...
X <- initializeX(W, Mu[1], probs[1], 2, tmax=20)
...
for(k in 1:(niter-1)){
  ...
  for(j in 1:popsize){
    Xother <- X[X[,2]!=j,]
    W <-updateW(W, Xother)
    irm <- buildirm(Xother, b=Beta[k], m = Mu[k], a=Alpha[k])
    Xt <- drawXt(Xother = Xother, irm = irm, W=W, p=probs[k], b=Beta[k], m=Mu[k], a=Alpha[k], initdist = initdist)
    
    Xt.path<- drawpath(Xt, Xother, irm, tmax)
     
    X <- updateX(X,Xt.path,j)  
  }
